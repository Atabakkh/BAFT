...
public class OptimizedSearch extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String name = request.getParameter("optimizedsearchGene");
	...
	public String[][] greedySearch(String geneName) {
		...
		if (name != null) {
			...
			try {
				...
				while (rs.next()) {
					...
					// boolean validGreadyValue =
					// Arrays.asList(network).contains(
					// rs.getString(3));
					boolean validGreadyValue = checkExistNode(network,
							rs.getString(3));
				...
				while (rs.next()) {
					...
					boolean validGreadyValue = checkExistNode(network,
							rs.getString(3));
					if (validGreadyValue) {
						writer.write("<edge id=\"" + rs.getRow()
	...
	public boolean checkExistNode(String network[][], String nodeName) {
		int len = network.length;
		for (int i = 0; i < len; i++) {
			if (network[i][1] != null)
				if (network[i][1].contains(nodeName)) {
					return true;
				}
		}
		return false;
